...
| Surface diagnostics | Warnings, recoverable tool failures, setup requirements, logs, and live LLM latency/context/retry details are visible from the UI. |
...
The Helm shows current LLM activity at a glance. Retries, timeouts, provider failures, context rejections, and cancellations appear immediately as compact notification cards; selecting a card opens a dedicated full-screen LLM Diagnostics workspace with call latency, attempt timing, exact selected-call messages, tool schemas, provider token usage, and bounded secret-redacted context, raw-response, routing, and application logs. Cards are delivered live and reconciled from bounded activity snapshots if the browser event connection briefly drops. While a chat turn is active, the input is locked and Send becomes Stop; stopping cancels the server-side request. Routine tasks assemble toward smaller soft context targets and include serialized function schemas in their accounting. The configured/provider context window remains the hard authority, so an irreducible request may safely exceed a soft target without being rejected while still below the real model ceiling.

Model changes are saved atomically and the settings UI shows the effective provider/model for every LLM task, including whether the winning value comes from the global route, a capability tier, or a task override. Choosing “use this base route for every task” clears only hidden provider/model endpoint overrides while preserving task-specific generation and context tuning. Saving a changed route stops active calls that captured the old configuration and the next request resolves against the new revision. Browser assets are content-versioned and HTML is never pinned in cache; an obsolete chat/settings controller is rejected with an explicit reload instruction instead of silently running against a newer backend.
