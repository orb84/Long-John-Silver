...
def test_compass_category_owned_settings() -> None:
    ...
    assert_contains(panel, "_categoryServiceSettingsPayload", "category service settings payload")
...
def test_advanced_contracts_panel_is_not_empty_or_confusing() -> None:
    ...
    assert_contains(manifest_panel, "Private config: config/categories/", "config file ownership row")
...
def test_tv_yaml_settings_are_runtime_owned() -> None:
    tv_config_yaml = read("config/category-config-templates/tv.yaml")
    tv_definition_yaml = read("config/category-definitions/tv.yaml")
    ...
    for key in (
        ...
        assert_contains(tv_config_yaml, key, f"tv private config template {key}")

    for key in ("metadata:", "tools:", "llm_guidance:", "lifecycle_policy:", "format_examples"):
        assert_contains(tv_definition_yaml, key, f"tv definition {key}")

    assert_contains(category_config, "category-definitions", "category definition directory support")
    assert_contains(category_config, "category-config-templates", "private config template directory support")
    assert_contains(category_config, "_live_config_payload", "save-time filter for private category config")
...
def test_category_services_are_manifest_declared() -> None:
    ...
    assert_contains(tv, "def provider_setup_requirements", "TV provider setup requirements")
    assert_contains(movie, "def provider_setup_requirements", "Movie provider setup requirements")
    for key in ("services.tmdb.api_key", "services.trakt.client_id", "services.plex.url", "services.plex.token", "services.opensubtitles.api_key"):
        assert_contains(base_category, key, f"shared media setup key {key}")
...
def test_category_creation_skill_points_to_category_owned_ui() -> None:
    ...
    assert_contains(guide, "Compass renders per-category paths, provider toggles, service credentials, tool policy", "skill Compass guidance")
...
def test_category_config_store_roundtrips_nested_sections() -> None:
    ...
    store = CategoryConfigStore(directory=ROOT / "config/categories", template_directory=ROOT / "config/category-config-templates", definition_directory=ROOT / "config/category-definitions")
    payload = {
        "category_id": "example",
        "enabled": True,
        ...
        "services": {"tmdb": {"api_key": "", "enabled": True}},
        "tools": {"allowed_generic": ["metadata_lookup"]},
        "download_profile": {"language": "Italian"},
        "llm_guidance": {"behavior": ["Use category rules."]},
    }
    ...
    assert flat["services"]["tmdb"]["enabled"] is True
    assert flat["tools"]["allowed_generic"] == ["metadata_lookup"]
    assert flat["download_profile"]["language"] == "Italian"
    assert flat["llm_guidance"]["behavior"] == ["Use category rules."]
    ...
    assert inflated["services"] == payload["services"]
    assert inflated["tools"] == payload["tools"]
    assert inflated["download_profile"] == payload["download_profile"]
    assert inflated["llm_guidance"] == payload["llm_guidance"]
