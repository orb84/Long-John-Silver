...
def _assert_scope_injected() -> None:
    plan = AgentPlan(
        ...
        steps=[PlanStep(id="search", tool_name="search_media_torrents", arguments={"name": "Example", "season": 5, "search_scope": "season_pack_preferred"})],
    )
    ...
    assert normalized.steps[0].arguments["search_scope"] == "bundle_preferred"
    assert normalized.constraints["download_search_scope"] == "bundle_preferred"

    whole_plan = AgentPlan(
        intent=Intent.DOWNLOAD,
        user_goal="Download the latest season of a show",
        steps=[PlanStep(id="search", tool_name="search_media_torrents", arguments={"name": "Example", "season": 5, "search_scope": "bundle_preferred"})],
    )
    ...
    assert whole_normalized.steps[0].arguments["search_scope"] == "bundle_preferred"
...
def main() -> None:
    ...
    assert "bundle/range" in IntentRouter._ROUTING_PROMPT.lower()
