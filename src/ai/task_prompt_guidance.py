...
class TaskPromptGuidance:
    ...
    def search_task_rules() -> str:
        ...
        return (
            ...
            "- Acquisition continuation: when ACTIVE GOAL STATE contains media result_set handles/category ownership, use search_media_torrents to refine that acquisition. Do not replace the queueable candidate workspace with raw web URLs or magnets.\n"
            ...
            "- Public web results are informational evidence only. They never become selectable/queueable download candidates until category search returns stable candidate handles.\n"
    ...
    def download_task_rules() -> str:
        ...
        return (
            ...
            "- Fresh media discovery: use category context/enquire_about_media, then search_media_torrents. Category words in the user's language are hints only: let enquiry/metadata verify identity, and ask the returned clarification_question if evidence is ambiguous or missing.\n"
            "- When enquire_about_media returns recommended_search_arguments, reuse those structured category/unit/language fields for search_media_torrents instead of reconstructing intent from English keywords.\n"
            ...
            "- Untracked requested item: verify it through enquire_about_media/search_media_torrents; do not manufacture a library/tracking record merely to search.\n"
            "- Tracking, recurring watches, and future-season automation are configuration workflows. Keep an immediate DOWNLOAD turn focused on discovery/selection/queueing; surface the separate setup action when the user explicitly asks for ongoing automation.\n"
            ...
            "- If completion_contract.follow_up_required=false and action_required=queue_download, the user's request is already clear: call queue_download with the supplied arguments immediately. Do not ask for confirmation, offer a menu, or turn a complete multi-unit result into a discussion.\n"
            "- Never infer missing or extra category units from candidate groups. Use category metadata/tool fields for unit counts and treat category-approved bundle/range coverage as evidence only when the tool exposes it. season_total_episode_count is the catalogue order; aired_episode_count and target_unit_labels are the released search target. Never describe the catalogue total as currently released.\n"
            "- After a verified queue receipt, answer in plain human language with the title, unit labels or count, requested language, and actual queued/already-active status. Keep it brief; omit result-set IDs, internal policy names, receipt jargon, and warnings when there is no real failure.\n"
