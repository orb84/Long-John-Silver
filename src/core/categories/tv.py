...
class TvShowCategory(TvMetadataInfoMixin, TvContextMixin, TvAgentSearchMixin, TvWorkflowMixin, CategoryMedia):
    ...
    def provider_setup_requirements(self, settings: 'Settings') -> list[CategorySetupRequirement]:
        """Return TV-owned provider and watch-state setup guidance."""
        return [
            CategorySetupRequirement(
                id="tmdb_api_key",
                label="TMDB TV metadata key",
                description="Canonical show titles, season lists, artwork, cast, genres, ratings, and IDs for TV workflows.",
                required=False,
                configured=bool(settings.tmdb_api_key),
                setting_key="tmdb_api_key",
                help_url="https://www.themoviedb.org/settings/api",
                severity="recommended",
                why_it_matters="TV can operate with degraded metadata, but season/episode and artwork quality improve with TMDB.",
                secret=True,
            ),
            CategorySetupRequirement(
                id="tvmaze_provider",
                ...
                required=False,
                configured=self.metadata_provider_enabled(settings, "tvmaze", True),
                setting_key="category_config.tv.metadata.providers.tvmaze.enabled",
                severity="recommended",
            ...
            CategorySetupRequirement(
                ...
                description="Optional account link for watched-state aware TV automation and recommendations.",
                required=False,
            ...
            CategorySetupRequirement(
                ...
                description="Optional Plex endpoint for TV library refresh and watched-state reconciliation.",
                required=False,
            ...
            CategorySetupRequirement(
                ...
                description="Optional subtitle search/download key for TV releases.",
                required=False,
    ...
    def matches_external_media_type(self, source: str, media_type: str) -> bool:
        """Map Plex episode records to the TV category inside the category boundary."""
        return source.lower() == 'plex' and media_type == 'episode'

    def library_file_records_from_scan(self, scanned: Any) -> list[dict[str, Any]]:
        """Expose TV local files with TV-owned season/episode selectors."""
    ...
    def file_record_matches_selector(
        ...
        *,
        ...
        """Match TV cleanup records using TV-owned season/episode fields."""
    ...
    async def enrich_taste_metadata(self, item: Any, context: Any) -> dict[str, Any] | None:
        """Return TV-owned metadata by delegating to the metadata mixin.

        Keeping this hook visible on the concrete category class makes the
        category contract explicit while the mixin owns the provider details.
        """
        return await super().enrich_taste_metadata(item, context)


    @property
    def search(self) -> TvSearchPatterns:
        """Search using the TvShowCategory provider contract.
        ...
        not in callers.
        """
        return TvSearchPatterns()

    @staticmethod
    def _parse_search_unit_label(label: str | None) -> tuple[int | None, int | None]:
        """Parse TV-owned search labels such as ``S01E02`` or ``Season 1``.

        This parser deliberately lives in the TV category.  The shared search
        pipeline treats labels as opaque category text and delegates here when
        a TV request needs coordinate-aware query building or validation.
        """
        if not label:
            return None, None
        m_ep = re.search(r'S0*(\d+)E0*(\d+)', str(label), re.IGNORECASE)
        if m_ep:
            return int(m_ep.group(1)), int(m_ep.group(2))
        m_x = re.search(r'\b0*(\d+)x0*(\d+)\b', str(label), re.IGNORECASE)
        if m_x:
            return int(m_x.group(1)), int(m_x.group(2))
        m_season = re.search(r'(?:Season|S)\s*0*(\d+)\b', str(label), re.IGNORECASE)
        if m_season:
            return int(m_season.group(1)), None
        return None, None

    def build_search_query(self, item: Any, unit_label: str | None, language: str | None) -> str:
        """Build the primary TV torrent query from an opaque category label."""
        season, episode = self._parse_search_unit_label(unit_label)
        if season is not None and episode is None:
            pack = self.search.build_pack_query(item.key, language or '', season)
            if pack:
                return pack
            return self.search._append_language(f'{item.key} Season {season} Complete', language)
        if season is not None and episode is not None:
            query = self.search.build_primary_query(
                item.key, language or '', {'last_season': season, 'last_episode': episode - 1}
            )
            if query:
                return query
        query = self.search.build_primary_query(item.key, language or '', None)
        if unit_label and not any(token in query for token in (str(unit_label), f'S{season or 0:02d}')):
            return self.search._append_language(f'{item.key} {unit_label}', language)
        return query

    def build_alternative_search_queries(self, item: Any, unit_label: str | None, language: str | None) -> list[str]:
        """Return TV release-name variants for the requested unit label."""
        season, episode = self._parse_search_unit_label(unit_label)
        queries: list[str] = []
        if season is not None and episode is not None:
            queries.extend([
                f'{item.key} S{season:02d}E{episode:02d}',
                f'{item.key} {season}x{episode:02d}',
                # Some episodes are only present inside multi-episode/season torrents.
                # TV owns this fallback; the generic search pipeline treats it
                # as just another category-provided query.
                f'{item.key} S{season:02d}E{episode:02d}-E{episode:02d}',
                f'{item.key} S{season:02d} Complete',
                f'{item.key} Season {season} Complete',
            ])
            if language and language.lower() != 'english':
                queries.append(f'{item.key} S{season:02d}E{episode:02d}')
        elif season is not None:
            queries.extend([
                f'{item.key} S{season:02d} Complete',
                f'{item.key} Season {season} Complete',
            ])
        queries.extend(self.search.build_alternative_queries(item.key, language or '', None))
    ...
    def validate_search_result_for_request(self, result: Any, item: Any, unit_label: str | None) -> bool:
        """Validate TV candidates against the requested season/episode label.

        Exact SxxEyy releases are ideal, but a season/range bundle can be a
        valid candidate when it contains the requested episode.  That logic
        stays here in the TV category rather than in generic torrent search.
        """
        season, episode = self._parse_search_unit_label(unit_label)
        title = str(getattr(result, 'title', '') or '')
        if self.validate_result(title, season, episode):
            return True
        if season is None:
            return False
        ...
        # Complete-series torrents are plausible containers for any requested
        # season/episode.  They stay eligible so the LLM/category evidence layer
        # can judge quality/size/language, and selective download will later
        # keep only matching files once torrent metadata is available.
        if pack.get('pack_type') == 'series_complete':
            return True

        season_start = self._safe_positive_int(pack.get('season_start')) or self._safe_positive_int(pack.get('season'))
        season_end = self._safe_positive_int(pack.get('season_end')) or season_start
        if season_start is None or season_end is None or not (season_start <= int(season) <= season_end):
            return False
        ...
        # Complete, implicit, multi-season, or complete-series bundle containing
    ...
    def download_coordinates_from_search_result(self, result: Any, item: Any, unit_label: str | None) -> dict[str, Any]:
        """Populate transitional download coordinates from TV-owned parsing."""
        ...
        return {'season': coordinates.get('season'), 'episode': coordinates.get('episode')}

    def unit_descriptor_from_agent_args(self, *, season: int | None = None, episode: int | None = None, **_: Any) -> dict[str, Any]:
        """Describe a TV unit from transitional assistant coordinates."""
        if season is None:
            return {'granularity': 'item', 'label': '', 'coordinates': {}}
        coordinates = {'season': int(season)}
        if episode is not None:
            coordinates['episode'] = int(episode)
            label = f"S{int(season):02d}E{int(episode):02d}"
            return {
                'granularity': 'episode',
                'label': label,
                'stable_key': label,
                'sort_key': [int(season), int(episode)],
        ...
        return {
            ...
            'stable_key': label,
            'sort_key': [int(season), 0],
            'coordinates': coordinates,
        }

    def unit_descriptor_from_search_result(self, result: Any, item: Any, unit_label: str | None) -> dict[str, Any]:
        """Return TV-owned unit descriptor for a torrent candidate.

        This is the only place shared queue/search code may obtain TV episode
        coordinates. The descriptor travels through result caches and download
        import context so downstream systems do not re-parse titles or invent
        ``SxxEyy`` keys themselves.
        """
        parsed = self.parse_name(str(getattr(result, 'title', '') or ''))
        label_season, label_episode = self._parse_search_unit_label(unit_label)

        # For bundle candidates, the torrent title describes the container
        # scope (for example S01-S03 Complete), while the user/request label
        # describes the actual target unit. Prefer the explicit request label
        # whenever available so a request for S02E05 inside an S01-S03 torrent
        # does not accidentally target S01E05.
        season = label_season if label_season is not None else (parsed.season if parsed and parsed.season is not None else getattr(item, 'last_season', None))
        episode = label_episode if label_season is not None else (parsed.episode if parsed and parsed.episode is not None else getattr(item, 'last_episode', None))
    ...
    def torrent_bundle_candidate_context(self, result: Any, item: Any | None = None, unit_label: str | None = None) -> dict[str, Any] | None:
        """Describe TV season/range packs as category-owned bundles."""
        title = str(getattr(result, 'title', '') or '')
        pack = TVBundleKnowledge.detect_season_pack(title)
        if not pack:
            return None
        unit_count = None
        if pack.get('start') is not None and pack.get('end') is not None:
            try:
                unit_count = max(1, int(pack['end']) - int(pack['start']) + 1)
            except (TypeError, ValueError):
                unit_count = None
        if unit_count is None and pack.get('pack_type') == 'multi_season':
            try:
                season_count = max(1, int(pack['season_end']) - int(pack['season_start']) + 1)
                unit_count = TVBundleKnowledge.approximate_episode_count(title, season_count)
            except (TypeError, ValueError, KeyError):
                unit_count = None
        if unit_count is None and pack.get('pack_type') == 'series_complete':
            unit_count = TVBundleKnowledge.approximate_episode_count(title, 3)
        if unit_count is None:
            # Provider episode counts are better when available; otherwise the
            # TorrentKnowledge estimate is deliberately conservative.
            unit_count = TVBundleKnowledge.approximate_episode_count(title, 1)

        scope = pack.get('scope') or 'season'
        return {
            'is_bundle': True,
            'bundle_type': 'tv_bundle',
            'scope': scope,
            'season': pack.get('season'),
            'season_start': pack.get('season_start'),
            'season_end': pack.get('season_end'),
            'unit_count': unit_count,
            'pack_type': pack.get('pack_type'),
            'can_select_files_after_metadata': True,
            'selection_note': 'May contain one or more TV seasons or episodes; exact requested units can be selectively downloaded after torrent metadata arrives.',
        }
    ...
    def estimate_bundle_unit_size_mb(
        ...
        *,
        ...
        """Estimate per-episode size for TV season/range bundles."""
        context = bundle_context or self.torrent_bundle_candidate_context(type('Probe', (), {'title': title})()) or {}
        count = context.get('unit_count')
        try:
            count = int(count) if count else None
        except (TypeError, ValueError):
            count = None
        return float(TVBundleKnowledge.estimate_per_episode_size_mb(total_size_bytes, title, episode_count=count))
    ...
    def unit_descriptor_from_file(self, file_path: str, parsed: Any | None = None, item_descriptor: dict[str, Any] | None = None) -> dict[str, Any]:
        """Describe a file inside a TV torrent by its parsed episode unit."""
        if parsed and getattr(parsed, 'season', None) is not None:
            return self.unit_descriptor_from_agent_args(
                season=getattr(parsed, 'season', None),
    ...
    def torrent_file_matches_target(
        ...
        *,
        ...
        """Select TV files matching requested episode or whole-season targets."""
        ...
        file_season = self._safe_positive_int(coordinates.get('season')) or self._safe_positive_int(getattr(parsed, 'season', None))
        file_episode = self._safe_positive_int(coordinates.get('episode')) or self._safe_positive_int(getattr(parsed, 'episode', None))
        ...
        for target in target_descriptors or []:
            ...
            target_season = self._safe_positive_int(target_coords.get('season'))
            target_episode = self._safe_positive_int(target_coords.get('episode'))
            granularity = str(target.get('granularity') or '')
            if target_season is not None and target_season != file_season:
                continue
            if granularity == 'season' and target_season == file_season:
                return True
            if target_episode is not None and file_episode == target_episode:
                return True
            if target.get('stable_key') and target.get('stable_key') == file_descriptor.get('stable_key'):
                return True
        return False
    ...
    def torrent_file_priority(
        ...
        *,
        ...
        """Prioritize selected TV episode files, ignore samples/extras."""
        lower = str(file_path or '').lower()
        if not selected or 'sample' in lower:
            return 0
        if Path(file_path).suffix.lower() in {'.srt', '.ass', '.ssa', '.sub', '.idx'}:
            return 2
        return 4
    ...
    def sort_cached_download_candidates(self, entries: list[dict[str, Any]], request_context: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        """Order cached TV batch entries by their category-owned unit descriptor."""
        def key(entry: dict[str, Any]) -> tuple[int, int, str]:
            """Rank cached entries by category-owned season/episode sort coordinates."""
            descriptor = ((entry.get('candidate') or {}).get('unit_descriptor') or {})
            sort_key = descriptor.get('sort_key') if isinstance(descriptor.get('sort_key'), list) else []
            season = self._safe_positive_int(sort_key[0]) if len(sort_key) > 0 else 0
            episode = self._safe_positive_int(sort_key[1]) if len(sort_key) > 1 else 0
            return (season or 0, episode or 0, str((entry.get('candidate') or {}).get('title') or ''))
    ...
    def candidates_represent_same_unit(self, first: dict[str, Any], second: dict[str, Any], request_context: dict[str, Any] | None = None) -> bool:
        """Return whether two TV candidates target the same logical episode/season."""
        first_desc = first.get('unit_descriptor') or {}
        second_desc = second.get('unit_descriptor') or {}
        return bool(first_desc.get('stable_key') and first_desc.get('stable_key') == second_desc.get('stable_key'))

    async def discovery_already_satisfied(self, item: Any, unit_label: str | None, context: Any | None = None) -> bool:
        """Skip duplicate TV discovery when canonical local files already contain the target."""
        season, episode = self._parse_search_unit_label(unit_label)
        if season is None or episode is None or context is None:
            return False
        db = getattr(context, 'db', None)
        if db is None:
            return False
        rows = await db.media.list_category_units(self.category_id, item.key, status='downloaded')
        for row in rows or []:
            try:
                if int(row.get('season') or 0) == int(season) and int(row.get('episode') or 0) == int(episode):
                    return True
            except (TypeError, ValueError):
                continue
        return False
    ...
    def quality_reference_for_search(self, item: Any, unit_label: str | None, context: Any | None = None) -> str:
        """Return TV-owned local quality context for torrent/LLM ranking."""
        metrics = self._local_episode_quality_metrics(item, unit_label, context)
        if not metrics:
            return ''
        avg_mb = metrics['avg_episode_size_mb']
        bitrate = metrics['avg_bitrate_kbps']
        count = metrics['episode_count']
        res = ', '.join(metrics.get('resolutions') or []) or 'unknown resolution'
        codecs = ', '.join(metrics.get('codecs') or []) or 'unknown codec'
        return (
            f'Existing TV library quality reference for "{item.key}": {count} local episode file(s) average '
            f'{avg_mb:.0f} MB/file; estimated average bitrate {bitrate} kbps using a 55-minute runtime. '
            f'Common local resolution(s): {res}; codec(s): {codecs}. '
            f'target_episode_size_mb={avg_mb:.0f}; target_bitrate_kbps={bitrate}. '
            'Prefer candidates reasonably close to this quality when language/resolution/seeders are acceptable; '
            'do not choose the smallest file merely because it is smallest unless the user asked for a small-size cap.'
        )

    def _local_episode_quality_metrics(self, item: Any, unit_label: str | None, context: Any | None = None) -> dict[str, Any] | None:
        """Estimate TV quality from canonical scan/file evidence."""
        season, _episode = self._parse_search_unit_label(unit_label)
        files: list[tuple[int | None, int | None, int, str]] = []
        scan_result = None
        try:
            scheduler = getattr(context, 'scheduler', None) if context is not None else None
            scan_result = scheduler.get_last_scan_result() if scheduler else None
        except Exception:
            scan_result = None
        if scan_result:
            for scanned in getattr(scan_result, 'items', []) or []:
                if getattr(scanned, 'category_id', None) != self.category_id:
                    continue
                if str(getattr(scanned, 'name', '')).lower() != str(item.key).lower():
                    continue
                for f in getattr(scanned, 'files', []) or []:
                    size = int(getattr(f, 'size_bytes', 0) or 0)
                    if size > 0:
                        files.append((getattr(f, 'season', None), getattr(f, 'episode', None), size, getattr(f, 'quality', '') or ''))
                break
        if not files:
            files.extend(self._crawl_library_episode_files_for_quality(item, context))
        if season is not None:
            season_files = [row for row in files if row[0] == season]
            if season_files:
                files = season_files
        sizes = [size for _s, _e, size, _q in files if size > 0]
        if not sizes:
            return None
        avg_bytes = sum(sizes) / len(sizes)
        qualities = ' '.join(q for *_rest, q in files if q)
        resolutions = sorted({m.group(0).lower() for m in re.finditer(r'2160p|1080p|720p|480p|4k', qualities, re.I)})
        codecs = sorted({m.group(0).lower() for m in re.finditer(r'h265|x265|hevc|h264|x264|av1|xvid', qualities, re.I)})
        return {
            'episode_count': len(sizes),
            'avg_episode_size_mb': avg_bytes / (1024 * 1024),
            'avg_bitrate_kbps': int((avg_bytes * 8) / (55 * 60) / 1000),
            'resolutions': resolutions,
            'codecs': codecs,
        }

    def _crawl_library_episode_files_for_quality(self, item: Any, context: Any | None = None) -> list[tuple[int | None, int | None, int, str]]:
        """Small fallback crawl used only when no recent canonical scan exists."""
        out: list[tuple[int | None, int | None, int, str]] = []
        try:
            settings = getattr(context, 'settings', None) if context is not None else None
            if not settings:
                return out
            root = Path(self.get_root_path(settings)).expanduser()
            show_dir = root / item.key
            if not show_dir.exists() or not show_dir.is_dir():
                return out
            video_exts = {'.mkv', '.mp4', '.avi', '.m4v', '.mov', '.mpg', '.mpeg', '.wmv'}
            for path in show_dir.rglob('*'):
                if not path.is_file() or path.suffix.lower() not in video_exts:
                    continue
                try:
                    parsed = self.parse_name(path.name)
                    out.append((parsed.season, parsed.episode, path.stat().st_size, f'{parsed.resolution or ""} {parsed.codec or ""}'))
                except OSError:
                    continue
        except Exception as exc:
            logger.debug(f"Unable to crawl TV quality reference for {getattr(item, 'key', item)}: {exc}")
        return out


    def create_item(self, key: str, **kwargs: object) -> object:
        """Create a tracked TV item without leaking model choices to API code."""
