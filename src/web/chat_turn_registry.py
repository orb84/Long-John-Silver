"""Session-scoped ownership for browser chat turns.

The browser can reconnect, open a second tab, or temporarily fall back to the
REST endpoint while an earlier WebSocket turn is still running.  Per-socket
``active_turn`` variables cannot protect the conversation from those cases.
This registry is the process-local authority for one active assistant task per
chat session, independent of transport connection.
"""
